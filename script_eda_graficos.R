...
grafico_barra <- df_outliers %>% 
  ...
  ggplot(aes(y = reorder(name, +homicides), x = homicides)) +  # Reordenar por variable en orden decreciente
...
grafico_dispersion <- df_outliers |> 
  ggplot(aes(x = avg_income, y = housing_costs, color = name)) + 
...
grafico_burbuja <- df_outliers |>
  mutate(etiqueta = paste("Condado: ", name,
                          "\nIngreso promedio: ", avg_income,
                          "\nCosto vivienda: ", housing_costs,
                          "\nHijos en pobreza: ", children_in_poverty, sep="")) %>% 
  ggplot(aes(x = housing_costs, y = avg_income, size = children_in_poverty, colour = name, text = etiqueta)) + 
